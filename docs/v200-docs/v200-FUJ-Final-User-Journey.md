...
The user wants visual exploration without memorizing CLI commands. They are new to the codebase. Or they want to show a colleague. Or they want to build intuition before writing queries.

**Tauri philosophy** (from ES-V200-User-Journey-Addendum): Tauri is a **visual CLI launcher**, not a replacement for terminal workflows. Power users graduate to pure CLI within 7 days. The app teaches CLI by showing the command it runs.
...
│   │    OPEN     │─────▶│   IMPORT    │─────▶│   EXPLORE   │        │
│   │             │      │             │      │             │        │
│   │ Double-click│      │ Point at    │      │ Click, zoom │        │
│   │ app, see UI │      │ project,    │      │ trace edges │        │
│   │             │      │ build graph │      │ see code    │        │
│   └─────────────┘      └─────────────┘      └─────────────┘        │
│       <1 sec                5-30 sec            Ongoing             │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  1.1 DOUBLE-CLICK      1.2 DB CHECK          1.3 FIRST VIEW         │
│  ┌────────────────┐   ┌────────────────┐    ┌────────────────┐      │
│  │ Parseltongue   │   │ Rust backend   │    │ Empty state    │      │
│  │ .app launches  │──▶│ checks for     │───▶│ OR             │      │
│  │ WebView opens  │   │ existing .db   │    │ Last dashboard │      │
│  └────────────────┘   └────────────────┘    └────────────────┘      │
│                                                                     │
│         1.4 EMPTY STATE                  1.5 RETURNING STATE        │
│         ┌───────────────────────┐        ┌───────────────────────┐  │
│         │ "No project loaded"   │        │ Dashboard with stats  │  │
│         │ [Import Project]      │        │ from last session     │  │
│         │ [Open Recent ▼]       │        │ [+ Add Project]       │  │
│         └───────────────────────┘        └───────────────────────┘  │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
2. WebView (WKWebView on macOS, WebView2 on Windows) initializes
3. Rust backend starts — this is `rust-llm-interface-gateway` in Tauri mode
4. Frontend React app loads from bundled assets (no network call)
5. React calls Tauri invoke: `get_codebase_ingestion_status`

**Tauri command**:
```rust
#[tauri::command]
async fn get_codebase_ingestion_status(
    state: tauri::State<'_, AppState>,
) -> Result<IngestionStatus, String> {
    match state.db.lock().await.as_ref() {
        None => Ok(IngestionStatus::NoDatabase),
        Some(db) => Ok(IngestionStatus::Ready {
            workspace: db.workspace_id.clone(),
            entity_count: db.entity_count(),
            last_ingested: db.last_ingested,
        }),
    }
}
```

**Branch A — No database** → Show empty state:
...
│                                                            │
│         ── or open recent ──                               │
│         /Users/dev/myapp          (3 days ago)             │
│         /Users/dev/other-project  (1 week ago)             │
│                                                            │
│                                                            │
└────────────────────────────────────────────────────────────┘
```

**Branch B — Database found** → Show last dashboard immediately (no loading state).

---

### L2 — Phase 2: Import (Detailed)

```text
┌─────────────────────────────────────────────────────────────────────┐
│                  PHASE 2: IMPORT — LEVEL 2                          │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  2.1 CLICK IMPORT     2.2 FILE PICKER        2.3 LANGUAGE DETECT    │
│  ┌────────────────┐   ┌────────────────┐    ┌────────────────┐      │
│  │ [Import        │   │ Native OS file │    │ Scan manifests:│      │
│  │  Project]      │──▶│ picker dialog  │───▶│ Cargo.toml     │      │
│  │ button         │   │ folder only    │    │ package.json   │      │
│  └────────────────┘   └────────────────┘    │ go.mod, etc.  │      │
│                                             └───────┬────────┘      │
│                                                     │               │
│  2.4 PROGRESS          2.5 CLI COMMAND SHOWN        │               │
│  ┌────────────────┐   ┌────────────────┐            │               │
│  │ Per-language   │◀──│ Show the exact │◀───────────┘               │
│  │ progress bars  │   │ CLI command    │                            │
│  │ streaming      │   │ being run      │                            │
│  └────────────────┘   └────────────────┘                            │
│           │                                                         │
│           ▼                                                         │
│  2.6 DASHBOARD APPEARS                                              │
│  ┌────────────────────────────────────────────────────────────┐     │
│  │ Stats, language breakdown, cross-boundary count            │     │
│  └────────────────────────────────────────────────────────────┘     │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
### L3 — Phase 2: Import (Implementation Detail)
...
```rust
#[tauri::command]
async fn open_folder_picker_dialog(
    window: tauri::Window,
...
User selects `/Users/dev/myapp`. Dialog closes. User is back in the app.

#### 2.3 Language Detection (Pre-scan, instant)

Before starting full ingest, quickly scan for manifests to show the user what's coming:

```rust
fn detect_project_languages(root: &Path) -> Vec<DetectedLanguage> {
    let mut langs = vec![];
    if root.join("Cargo.toml").exists()   { langs.push(DetectedLanguage::Rust); }
    if root.join("package.json").exists() { langs.push(DetectedLanguage::TypeScript); }
    if root.join("go.mod").exists()       { langs.push(DetectedLanguage::Go); }
    if root.join("pyproject.toml").exists() || root.join("setup.py").exists() {
        langs.push(DetectedLanguage::Python);
    }
    // Walk one level for frontend/ subdirs
    langs
}
```

#### 2.4 + 2.5 Progress Screen with CLI Command Shown

**Critical Tauri philosophy moment**: The UI shows the exact CLI command being run. This teaches the user the CLI.

```text
┌────────────────────────────────────────────────────────────┐
│  Parseltongue v2.0                                         │
│                                                            │
│  Importing: /Users/dev/myapp                               │
│                                                            │
│  🦀 Rust (backend/):                                       │
│  ████████████████░░░░  78%  (234/312 files)                │
│                                                            │
...
│  ──────────────────────────────────────────────────────    │
│  Running:                                                  │
│  $ rust-llm-interface-gateway ingest /Users/dev/myapp      │
│                                                            │
...
Each language runs tree-sitter extraction in parallel using `tokio::spawn`. Rust files additionally run rust-analyzer semantic enrichment after tree-sitter completes.

**Streaming progress via Tauri events**:
```rust
// Backend emits progress events
window.emit("ingest_progress", IngestProgress {
...
// Frontend listens
const unlisten = await listen('ingest_progress', (event) => {
...
```text
┌────────────────────────────────────────────────────────────────────┐
│  Parseltongue v2.0              myapp   [+ Add Project]  ⚙️         │
├──────────────────────────────────────────────────────────────────┤  │
│                                                                  │  │
│  📊 Codebase Overview          myapp-20260221-143022             │  │
│  ┌──────────────────────────────────────────────────────────┐   │  │
│  │ Entities: 846    Edges: 5,418    Languages: 3            │   │  │
│  │                                                          │   │  │
│  │ 🦀 Rust:         312 entities   ██████████████████ 37%   │   │  │
│  │ ⚛️  TypeScript:  445 entities   ████████████████████ 53% │   │  │
│  │ 📜 JavaScript:    89 entities   ████ 10%                 │   │  │
│  └──────────────────────────────────────────────────────────┘   │  │
│                                                                  │  │
│  🌐 Cross-Language Boundaries: 31                               │  │
│     • Rust → TypeScript (HTTP): 23 high, 8 medium               │  │
│     • TypeScript → JavaScript (import): 12                      │  │
│                                                                  │  │
│  ──────────────────────────────────────────────────────────      │  │
│                                                                  │  │
...
│                                                                  │  │
│  Quick actions:                                                  │  │
│  [🌐 Boundaries]  [🔄 Cycles]  [🔥 Hotspots]  [📊 Graph]         │  │
│                                                                  │  │
└──────────────────────────────────────────────────────────────────┘
```
...
### L2 — Phase 3: Explore (Detailed)

```text
┌─────────────────────────────────────────────────────────────────────┐
│                  PHASE 3: EXPLORE — LEVEL 2                         │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  3A SEARCH            3B BOUNDARIES         3C ENTITY DETAIL        │
│  ┌────────────────┐   ┌────────────────┐    ┌────────────────┐      │
│  │ Type "auth"    │   │ See all HTTP / │    │ Click entity   │      │
│  │ get 12 results │   │ FFI / WASM     │    │ see source,    │      │
│  │ all languages  │   │ crossings table│    │ callers, edges │      │
│  └────────────────┘   └────────────────┘    └────────────────┘      │
│                                                                     │
│  3D BLAST RADIUS      3E CODE VIEW          3F EXPORT               │
│  ┌────────────────┐   ┌────────────────┐    ┌────────────────┐      │
│  │ Visual graph   │   │ Live source    │    │ JSON/Mermaid/  │      │
│  │ cross-language │   │ from disk      │    │ DOT export     │      │
│  │ hops slider    │   │ (never cached) │    │                │      │
│  └────────────────┘   └────────────────┘    └────────────────┘      │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
### L3 — Phase 3A: Search (Implementation Detail)

User clicks the search box, types `auth`.

**Tauri command**:
```rust
#[tauri::command]
async fn search_entities_fuzzy_query(
    query: String,
    state: tauri::State<'_, AppState>,
) -> Result<Vec<EntitySummary>, String> {
    let db = state.db.lock().await;
    let db = db.as_ref().ok_or("No database loaded")?;
    db.search_entities_fuzzy(&query)
...
│  🦀 Rust (4)                                                       │
│  ┌────────────────────────────────────────────────────────────┐   │
│  │ fn   handle_auth_request   backend/src/api/auth.rs:45      │   │
│  │ fn   auth_middleware        backend/src/middleware.rs:12   │   │
│  │ fn   validate_user          backend/src/auth/logic.rs:34   │   │
│  │ struct LoginCredentials     backend/src/auth/types.rs:8    │   │
│  └────────────────────────────────────────────────────────────┘   │
│                                                                    │
│  ⚛️  TypeScript (6)                                                │
│  ┌────────────────────────────────────────────────────────────┐   │
│  │ fn        fetchAuthData       frontend/src/api/auth.ts:12  │   │
│  │ hook      useAuth             frontend/src/hooks/useAuth.ts│   │
│  │ component AuthGuard           frontend/src/components/...  │   │
│  │ component LoginForm           frontend/src/components/...  │   │
│  │ interface AuthResponse        frontend/src/types.ts:45     │   │
│  │ interface LoginCredentials    frontend/src/types.ts:52     │   │
│  └────────────────────────────────────────────────────────────┘   │
│                                                                    │
│  📜 JavaScript (2)                                                 │
│  ┌────────────────────────────────────────────────────────────┐   │
│  │ fn  parseAuthToken    frontend/src/utils/auth.js:34         │   │
│  │ fn  formatAuthError   frontend/src/utils/auth.js:56         │   │
│  └────────────────────────────────────────────────────────────┘   │
│                                                                    │
│  🌐 1 cross-language connection found:                            │
│     Rust handle_auth_request ──HTTP──▶ TS fetchAuthData (0.90)   │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
```

**Key moment**: The cross-language connection is surfaced directly in search results. User immediately sees the Rust→TypeScript link without asking for it.

---

### L3 — Phase 3B: Boundaries View (Implementation Detail)

User clicks `[🌐 Boundaries]`.

```text
┌────────────────────────────────────────────────────────────────────┐
│  Cross-Language Boundaries (31 found)                              │
├────────────────────────────────────────────────────────────────────┤
│  Filter: [All ▼]  [High only]  [HTTP] [FFI] [WASM] [Queue]        │
│                                                                    │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │ FROM                      TO                    TYPE  CONF   │  │
│  ├──────────────────────────────────────────────────────────────┤  │
│  │ 🦀 handle_auth_request   ⚛️  fetchAuthData       HTTP  0.90 │  │
│  │    backend/src/api/       frontend/src/api/       POST        │  │
│  │    auth.rs:45             auth.ts:12              /api/auth   │  │
│  ├──────────────────────────────────────────────────────────────┤  │
│  │ 🦀 get_user_profile       ⚛️  fetchUserProfile    HTTP  0.85 │  │
│  │    backend/src/api/       frontend/src/api/       GET         │  │
│  │    users.rs:89            users.ts:23             /api/user   │  │
│  ├──────────────────────────────────────────────────────────────┤  │
│  │ ⚛️  Component.tsx         📜 sharedUtils.js       IMPORT 1.0 │  │
│  │    frontend/src/comp/     frontend/src/utils/                 │  │
│  │    LoginForm.tsx:3        shared.js:1                         │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                    │
│  ⚠️  8 medium-confidence boundaries (0.60-0.79)  [Show]            │
│                                                                    │
│  ──────────────────────────────────────────────────────────────    │
│  CLI equivalent:                                                   │
│  $ rust-llm-interface-gateway boundaries --min-confidence 0.80     │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
```

User clicks on the `handle_auth_request → fetchAuthData` row.

---

### L3 — Phase 3C: Entity Detail View (Implementation Detail)

Clicking the row opens a side panel:
...
│  ──────────────────────────────────────────────────────────────    │
│                                                                    │
│  📍 backend/src/api/auth.rs  lines 45–67                          │
│  🏷️  fn  •  rust  •  async  •  89 tokens                          │
│                                                                    │
│  ┌── Source (live from disk) ──────────────────────────────────┐   │
│  │  45│ pub async fn handle_auth_request(                      │   │
│  │  46│     State(db): State<DbPool>,                          │   │
│  │  47│     Json(credentials): Json<LoginCredentials>,         │   │
│  │  48│ ) -> Result<Json<AuthToken>, AppError> {               │   │
│  │  49│     let user = validate_user(&db, &credentials).await?;│   │
│  │  50│     let token = generate_token(&user)?;                │   │
│  │  51│     Ok(Json(token))                                    │   │
│  │  52│ }                                                      │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                    │
│  🔗 Calls (Rust-typed via rust-analyzer):                         │
│  ├── validate_user    [Direct]       backend/src/auth/logic.rs    │
│  └── generate_token   [Direct]       backend/src/auth/token.rs    │
│                                                                    │
│  📞 Called by:                                                     │
│  └── auth_middleware   [calls]       backend/src/middleware.rs     │
│                                                                    │
│  🌐 Cross-language (HTTP boundary, conf=0.90):                    │
│  └── ⚛️  fetchAuthData  [http_boundary]  frontend/src/api/auth.ts │
│                                                                    │
│  📁 Public module context (same file + both public):              │
│  ├── fn   login_handler  [public_module_context]  (same file)     │
│  ├── use  axum::Json     [public_module_context]  (import feeds)  │
│  └── struct AuthRouter   [public_module_context]  (same file)     │
│                                                                    │
│  📂 Also in same file (private):                                   │
│  └── fn helper_parse_token  [shared_context]  (private, same file)│
│                                                                    │
│  [🔥 Blast Radius]  [📋 Copy Key]  [🖥️ Open in Editor]            │
│                                                                    │
│  ──────────────────────────────────────────────────────────────    │
│  CLI equivalent:                                                   │
│  $ rust-llm-interface-gateway entity \                             │
│      "rust|||fn|||backend::api::auth|||handle_auth_request|||..."  │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
```

**Critical detail — two tiers of co-location**:

- **Public module context** shows only entities that are *also* exported/public — these form the module's API surface together. An import statement listed here means "this import exists because this public function needs it." An interface listed here means "this interface is the contract this function implements or returns."
- **Private same-file** shows private helpers that share the file but are *not* part of the public surface.

This distinction is invisible without a graph. Grep gives you all names in a file. The graph tells you *which ones are semantically bound together* because they're both part of what this module exposes to the world.

**Error state if file moved**: If `backend/src/api/auth.rs` has been deleted since ingest, the Source panel shows:
```text
  ⚠️  File not found: backend/src/api/auth.rs
  The source was valid at ingest time. Re-ingest to refresh.
```
Never silently shows stale source. (Gate G3)

---

### L3 — Phase 3D: Blast Radius (Implementation Detail)

User clicks `[🔥 Blast Radius]`.

```text
┌────────────────────────────────────────────────────────────────────┐
│  Blast Radius: handle_auth_request                                 │
├────────────────────────────────────────────────────────────────────┤
│  Hops: [●━━━━━━━━━] 1   2   3   4   5   6                          │
│  Show: [☑ Rust] [☑ TypeScript] [☑ JavaScript]  [☑ HTTP crossings] │
├────────────────────────────────────────────────────────────────────┤
│                                                                    │
│          ┌──────────────────────┐                                  │
│          │    🦀 RUST LAYER     │                                  │
...
│          │  [auth_middleware]   │                                  │
│          │        ↓             │                                  │
│          │  [handle_auth_req] ◀─┼── TARGET                         │
│          │        │             │                                  │
│          └────────┼─────────────┘                                  │
│                   │ HTTP POST /api/auth  (conf=0.90)               │
│                   ▼                                                │
│          ┌──────────────────────┐                                  │
│          │   ⚛️  REACT LAYER    │                                  │
│          │                      │                                  │
...
│          └──────────────────────┘                                  │
│                                                                    │
│  Impact: 🦀 2 Rust  •  ⚛️ 4 TypeScript  •  Total: 6               │
│  Crossings: 1 (Rust→React via HTTP)                               │
│                                                                    │
│  ──────────────────────────────────────────────────────────────    │
│  CLI equivalent:                                                   │
│  $ rust-llm-interface-gateway blast-radius \                       │
│      "rust|||fn|||..." --hops 2                                    │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
```

Dragging hops slider from 2 → 3 triggers a new Tauri invoke (debounced 300ms). The graph updates in place.

**Tauri command**:
```rust
#[tauri::command]
async fn analyze_blast_radius_impact(
    key: String,
    hops: u32,
    state: tauri::State<'_, AppState>,
) -> Result<BlastRadiusResult, String> {
    let db = state.db.lock().await;
    let db = db.as_ref().ok_or("No database")?;
    // Runs Datalog transitive reachability via rust-llm-graph-reasoning
    db.blast_radius(&key, hops).map_err(|e| e.to_string())
}
```

**Behind the scenes — Ascent Datalog**:
```rust
ascent! {
    struct BlastRadiusProgram;

    relation edge(EntityKey, EntityKey, EdgeKind, f32);
    relation target(EntityKey);
    relation reachable(EntityKey, u32); // (entity, hop_count)

    reachable(b, 1) <-- target(a), edge(a, b, _, _);
    reachable(c, n+1) <-- reachable(b, n), edge(b, c, _, _), (n < MAX_HOPS);
}
```

---

### L3 — Phase 3E: Code View and Cross-Language Type Insight

User clicks on the TypeScript `fetchAuthData` entity in the blast radius graph.

Detail panel opens for the TypeScript function. Then user clicks `[🌐 Show Type Alignment]`:
...
├────────────────────────────────────────────────────────────────────┤
│                                                                    │
│  RUST: LoginCredentials (backend/src/auth/types.rs:8)             │
│  ┌──────────────────────────────────────────────┐                 │
...
│  └──────────────────────────────────────────────┘                 │
│                          ↕ HTTP JSON                               │
│  TYPESCRIPT: LoginCredentials (frontend/src/types.ts:52)          │
│  ┌──────────────────────────────────────────────┐                 │
...
│  └──────────────────────────────────────────────┘                 │
│                                                                    │
│  ✅ Shape: ALIGNED (field names match, types compatible)           │
│  ✅ Serialization: JSON via serde_json ↔ JSON.parse               │
│                                                                    │
│  ⚠️  AuthToken: Rust u64 id field → TypeScript number             │
│     Risk: JS number is f64. Safe only if id < 2^53 (~9 quadrillion)│
│     Recommendation: Serialize as string in Rust, parse as BigInt  │
│     in TypeScript if IDs may exceed 2^53.                         │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
```

This is the **killer value-add for polyglot codebases** — surfacing type alignment risks automatically at the language boundary.

---

### L3 — Phase 3F: Export and CLI Graduation
...
│  ☐ Source snippets (large)                          │
│                                                      │
│  [Export]                        [Cancel]            │
└──────────────────────────────────────────────────────┘
```

After export, the app shows the CLI command that produces the same output:

```text
┌──────────────────────────────────────────────────────────────────┐
│  ✓ Exported: myapp-report-20260221.json                          │
│                                                                  │
│  You can produce this same report from terminal:                 │
│  $ rust-llm-interface-gateway export \                           │
│      --format json \                                             │
│      --include-boundaries \                                      │
│      --include-type-alignment \                                  │
│      --out myapp-report.json                                     │
│                                                                  │
│  💡 Power tip: pipe to jq for custom queries                     │
│  $ rust-llm-interface-gateway export --format json | \           │
│      jq '.boundaries[] | select(.confidence >= 0.8)'            │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```

**Tauri graduation moment**: By Day 7, the user knows the CLI. They stop opening the app and work directly in terminal. The app has done its job.
