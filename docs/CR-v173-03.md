...
### Feature 6.1: Multi-Provider LLM Client with Streaming
...
### Feature 6.2: Agent Runtime with Steering & Follow-Up Queues
...
### Feature 6.3: Full MCP Client with Discovery, OAuth & Tool Caching
...
### Feature 6.4: Built-in LSP Integration (diagnostics, definition, references, rename, format)
...
### Feature 6.5: Swarm Extension — DAG-Based Multi-Agent Orchestration
...
### Feature 6.6: Task System with Git Worktree Isolation
...
### Feature 6.7: Session Compaction with File Operation Tracking
...
### Feature 6.8: Extension System (Custom Tools, Commands, Skills, Slash Commands)
...
### Feature 6.9: Native Rust Bindings (grep, glob, image, shell, PTY, highlight)
...
### Feature 6.10: Stats Dashboard (Local Observability)
...
## oh-my-pi Summary Table

| # | Feature | vs Parseltongue | Parseltongue Needed? | Shreyas: User Segment | Shreyas: Differentiation |
|---|---------|-----------------|----------------------|-----------------------|--------------------------|
| 6.1 | Multi-Provider LLM Client | PT outputs JSON, not LLM calls | No — intentionally excluded (P3-6 rationale) | Individual developers | Commoditized (every AI tool does this) |
| 6.2 | Agent Runtime + Steering | PT is stateless HTTP API | No — orchestrator concern | Individual developers | Neutral (good UX, not defensible) |
| 6.3 | MCP Client + OAuth + Caching | PT should be MCP SERVER | Yes — pt09 MCP server (P0-1, already planned) | Enterprise + Solo devs | Leverage (ecosystem unlock) |
| 6.4 | LSP Integration | PT uses tree-sitter (static) | Partial — tree-sitter covers 80% | Professional developers | Neutral (LSP is standard) |
| 6.5 | Swarm DAG Orchestration | PT HAS the graph data for this | Future — expose wave planner endpoint | Platform teams | Vitamin (nice, not essential) |
| 6.6 | Git Worktree Isolation | Not applicable | No — orchestrator concern | Enterprise teams | Defensible (complex to build) |
| 6.7 | Session Compaction | PT has `/smart-context-token-budget` | Evolve — track analysis sessions | All users | Neutral (context management) |
| 6.8 | Extension System | PT has no plugins | No — platform concern (P3) | Power users | Vitamin (ecosystem play) |
| 6.9 | Rust Native Bindings | PT is already 100% Rust | No — PT already has this natively | Performance-sensitive | Leverage for PT (native advantage) |
| 6.10 | Stats Dashboard | No usage tracking | Optional P2 — analysis telemetry | Enterprise | Vitamin (nice to have) |

### Shreyas Doshi LNO Assessment for oh-my-pi

**Leverage features** (move needle): MCP server (6.3 validates our P0-1 priority — every serious tool needs MCP)
...
oh-my-pi validates our existing roadmap rather than introducing new P0 priorities. It's an **MCP client** (consumer) while Parseltongue should be an **MCP server** (provider). The swarm DAG orchestration (6.5) is the most interesting intersection — oh-my-pi builds DAGs for agent execution, while Parseltongue already computes dependency DAGs for code. A future `/topological-execution-wave-planner` endpoint could make Parseltongue the graph engine that tools like oh-my-pi call for intelligent task decomposition.
...
*Document updated 2026-02-15 with oh-my-pi analysis. Original document generated by Agent 03 (Pass 3) on 2026-02-14. Analysis based on Parseltongue v1.6.5 (API v1.6.5, 24 endpoints, 44,890 entities, 385,560 edges) running on port 7777.*
